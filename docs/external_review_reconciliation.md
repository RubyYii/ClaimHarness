...
| 6 | UI 的 provider 选择容易让人误以为会改变 ProblemBridge/验证 | **已完成** | [`apps/problem_bridge_wizard.py`](../apps/problem_bridge_wizard.py)、[`claim_harness/cli.py`](../claim_harness/cli.py)、[`claim_harness/llm.py`](../claim_harness/llm.py)、[`MODEL_PROVIDER_GUIDE.md`](../MODEL_PROVIDER_GUIDE.md) | 大多数工作台步骤仍为 deterministic local workflow。新增的“证据门控构建”只允许 `mock` 或官方 OpenAI GPT-5.6：远程模型仅提出结构化候选能力声明，Pydantic 校验与 ClaimHarness 证据门保持确定性；UI 明示运行来源且不允许 provider 改写审计状态。 |
