...
[项目主页](../README.zh-CN.md) · [English reference](reference.md) · [文档索引](project_map.md)

[静态演示](static_showcase/zh-CN.html)

第一节介绍当前的需求整理与两份说明交付；后续章节供需要原有工具、命令行和维护细节时查阅。所有命令都从仓库根目录运行。

[当前实现与 14 类问题状态](external_review_reconciliation.md) · [OCR 安装](../OCR_SETUP.md)
...
页面把当前问题放进独立卡片，桌面右侧说明两份产出的用途，手机上移到输入区下方。确认页按工作背景与期望结果分组，交付页提供独立文档预览；浅色主题与绿色主按钮保持一致。参见[布局调整与验证记录](layout_polish_2026-09-22.md)。
...
**有结果后再核查：** 粘贴正文或上传 Markdown/TXT，再提供 CSV 结果表。每条发现保留声明 ID 与来源位置；回答追问只记录行动，不会自动改变核查结论。核查仍以英文数值声明为主，提取是否完整需要人工检查；没有提取到声明不表示通过。生成的需求说明不是实验凭据，也不代表接收方已同意。本地提问是预设引导，并未实现任意领域的语义理解或自动执行任务。参见[跨专业需求交付设计](cross_disciplinary_handoff.md)与[核查流程设计](unified_workbench_design.md)。
...
不会自动覆盖。需要恢复或替换时，请按 [运行生命周期说明](v0.4_upgrade.md#safe-output-lifecycle)
提供对应的项目和运行身份。
...
OCR 图文安装说明见 [OCR_SETUP.md](../OCR_SETUP.md)，也可以直接打开本地网页 [docs/ocr_setup.html](ocr_setup.html)。
...
可以打开中文静态展示页查看：[`docs/static_showcase/zh-CN.html`](static_showcase/zh-CN.html)。
...
v0.4.1 统一了抽取与高风险识别词表，以及检索与验证中的表格比较规则。数值冲突会指出“正文值、表格值、行/单元格”；模型、比较对象、实验、split、单位、否定和不支持的数值限定不明确时转人工澄清。报告直接展示问题、原因、下一步动作，并注明“抽取完整性未知”。详见[本次优化记录](audit_core_optimization_2026-09-05.md)。
